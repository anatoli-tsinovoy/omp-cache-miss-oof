...
Restart OMP after installation. This extension requires OMP 18.0.11 or newer.
...
The extension subscribes to `message_end` and applies the same explicit-cache warm-to-cold conditions as OMP's cache-invalidation marker. The detector runs locally to avoid loading OMP's UI module during plugin installation.
